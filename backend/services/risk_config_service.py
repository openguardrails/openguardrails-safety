...
class RiskConfigService:
    ...
    def _resolve_workspace_id(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None) -> Optional[str]:
        """Resolve workspace_id from workspace_id, application_id, or tenant_id.

        0. If workspace_id provided -> use directly
        1. If application_id provided -> resolve to workspace_id via app's workspace
        2. If only tenant_id provided -> find global workspace
        """
        if workspace_id:
            return workspace_id
        ...
        if application_id:
            ...
            if workspace_id:
                return workspace_id
            # If app has no workspace, try tenant's global workspace
            if tenant_id:
                return get_global_workspace_id(self.db, tenant_id)
            # Try to get tenant_id from the application
            app = self.db.query(Application.tenant_id).filter(Application.id == application_id).first()
            if app and app.tenant_id:
                return get_global_workspace_id(self.db, str(app.tenant_id))
            return None
    ...
    def get_user_risk_config(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None) -> Optional[RiskTypeConfig]:
        """Get user risk config by workspace_id (resolved from workspace_id, application_id, or tenant_id)"""
        try:
            if not application_id and not tenant_id and not workspace_id:
                raise ValueError("Either workspace_id, tenant_id, or application_id must be provided")

            workspace_id = self._resolve_workspace_id(tenant_id=tenant_id, application_id=application_id, workspace_id=workspace_id)
            if workspace_id:
    ...
    def create_default_risk_config(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None) -> RiskTypeConfig:
        """Create default risk config for workspace (all types default enabled)"""
        try:
            if not application_id and not tenant_id and not workspace_id:
                raise ValueError("Either workspace_id, tenant_id, or application_id must be provided")
            ...
            workspace_id = self._resolve_workspace_id(tenant_id=tenant_id, application_id=application_id, workspace_id=workspace_id)
            if not workspace_id:
                raise ValueError(f"Could not resolve workspace_id for tenant_id={tenant_id}, application_id={application_id}")

            # Resolve tenant_id for the record
            resolved_tenant_id = tenant_id
            if not resolved_tenant_id and application_id:
                app = self.db.query(Application.tenant_id).filter(Application.id == application_id).first()
                if app:
                    resolved_tenant_id = str(app.tenant_id)
            ...
            logger.info(f"Created default risk config for workspace_id={workspace_id}")
            return config
        except Exception as e:
    ...
    def update_risk_config(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None, config_data: Dict = None) -> Optional[RiskTypeConfig]:
        """Update user/application risk config"""
        try:
            if not config_data:
                config_data = {}

            config = self.get_user_risk_config(tenant_id=tenant_id, application_id=application_id, workspace_id=workspace_id)
            if not config:
                config = self.create_default_risk_config(tenant_id=tenant_id, application_id=application_id, workspace_id=workspace_id)
    ...
    def get_enabled_risk_types(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None) -> Dict[str, bool]:
    ...
    def is_risk_type_enabled(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None, risk_type: str = None) -> bool:
        """Check if specified risk type is enabled"""
        enabled_types = self.get_enabled_risk_types(tenant_id=tenant_id, application_id=application_id, workspace_id=workspace_id)
        return enabled_types.get(risk_type, True)  # Default enabled
    
    def get_risk_config_dict(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None) -> Dict:
    ...
    def update_sensitivity_thresholds(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None, threshold_data: Dict = None) -> Optional[RiskTypeConfig]:
    ...
    def get_sensitivity_threshold_dict(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None) -> Dict:
    ...
    def get_sensitivity_thresholds(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None) -> Dict[str, float]:
    ...
    def get_sensitivity_trigger_level(self, tenant_id: str = None, application_id: str = None, workspace_id: str = None) -> str:
